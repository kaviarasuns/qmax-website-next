...
export function CapabilitiesSection({
  ...
  const buildPath = useCallback(
    (width: number, baseY: number, x1: number, x2: number, tabHeight: number) => {
  ...
  const startAnimation = useCallback(
    (newLeft: number, newRight: number) => {
      ...
      // Anchor on the current visual position so an interrupt mid-flight
      // continues smoothly from where the indicator actually is.
  ...
  }, [activeIdx, capabilities.length, getTabBounds, redrawPath, startAnimation]);
