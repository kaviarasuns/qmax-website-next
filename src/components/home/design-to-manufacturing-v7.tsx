...
const STAGES: Stage[] = [
  {
    ...
    bullets: [
      ...
      "System architecture & chipset selection",
      "Feasibility & technical risk assessment",
      "Target BOM cost architecture",
    ],
    media:
  ...
  },
  {
    ...
    body: "Transforming architectural blueprints into high-performance circuit designs.",
    bullets: [
      "Schematic capture & library design",
      "Component lifecycle engineering",
      "Power & mixed-signal management",
  ...
  },
  {
    ...
    bullets: [
      "Multi-layer layout & stack-up design",
      "High-speed routing & constraint matching",
      "SI, PI, and thermal simulations",
      "DFM & DFA compliance auditing",
    ],
  ...
  },
  {
    ...
    body: "Bringing silicon to life with secure, reliable, and low-latency code.",
    bullets: [
      "Bootloader, BSP, & driver development",
      "RTOS & Embedded Linux deployment",
      "Connectivity protocols & edge compute",
      "Cloud integration & OTA pipelines",
    ],
    media:
  ...
  },
  {
    ...
    bullets: [
      ...
      "3D enclosure CAD modeling",
      "FEA structural & thermal simulation",
      "Ingress protection (IP) sealing design",
    ],
    media:
  ...
  },
  {
    ...
    body: "Stress-testing hardware against global regulatory frameworks and harsh conditions.",
    bullets: [
      "Pre-compliance EMI/EMC scanning",
      "Global regulatory certification management",
  ...
  },
  {
    ...
    body: "Scaling up manufacturing smoothly with verified yields and zero factory surprises.",
    bullets: [
      "Production-ready DFx package release",
      "EMS vendor audits & tooling setup",
      "EVT / DVT / PVT pilot build oversight",
      "Yield optimization & EOL test deployment",
    ],
...
/*  COMPONENT */
export default function DesignToManufacturingV7() {
  ...
  useLayoutEffect(() => {
    ...
    // pointer is over the section — anywhere else the page scrolls natively.
    ...
    const pointerInSection = () => {
      ...
      // Test against the inner 1280px content column, not the full-width
      // wrapper, so the empty side gutters pass scroll through natively.
      const r = (sectionRef.current ?? wrapper).getBoundingClientRect();
      return (
        pointerPos.x >= r.left &&
    ...
    const ctx = gsap.context(() => {
      trigger = ScrollTrigger.create({
        ...
        onToggle: (self) => {
          ...
          pendingDir = 0;
          commit(entryIndex);
          // Pointer outside the section → don't trap the scroll; hop straight
          // over the pin so the page keeps its native flow.
          if (!pointerInSection()) {
            const target =
              self.direction === -1
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      // Mouse outside the section: leave this wheel event to native scrolling
      // and unpark the pin in the same direction.
      if (!pointerInSection()) {
